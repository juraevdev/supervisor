  ...
  return (
    <header className="w-full dark:bg-gray-900 text-gray-800 dark:text-white shadow-md px-4 py-3 flex justify-between items-center relative z-50">
      ...
      <nav className="flex flex-row md:flex gap-6 items-center">
        ...
        <Link to="/todo" className="text-white no-underline hover:text-blue-500">Tasks</Link>
        {!isAuthenticated ? (
          <Link to="/sign-in" className="text-white no-underline hover:text-blue-500">Sign in</Link>
        ) : (
          <div className="relative">
            ...
            {showPopup && (
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden z-50">
                <Link to="/profile" className="block px-4 py-2 no-underline text-black dark:hover:bg-gray-500">Profile</Link>
                <Link to="/request" className="block px-4 py-2 no-underline text-black dark:hover:bg-gray-500">Change Password</Link>
                <button onClick={handleLogout} className="w-full text-left text-black px-4 py-2 dark:hover:bg-gray-500">Leave</button>
              </div>
            )}
          </div>
        )}
